...
fn work() -> Result<()> {
    ...
    let configs_bytes_path = cwd.parent().unwrap().join("configs/src/bytes");
    ...
    mod_content += "use utils::constants::config_bytes_folder;\n\n";
    ...
    mod_content += "pub struct BytesConfigsData {\n";
    for module in mod_list.iter() {
    ...
    mod_content += "impl BytesConfigsData {\n";
    mod_content += "    pub async fn load_all() -> anyhow::Result<Self> {\n";
    mod_content += "        let config_path = config_bytes_folder();\n";

    for module in mod_list.iter() {
        if let Some(data_struct) = mod_data.get(module) {
            mod_content += &format!(
                "        let {} = {}::{}::load_data(config_path);\n",
                data_struct.to_snake_case().to_lowercase(),
                module,
                data_struct
            );

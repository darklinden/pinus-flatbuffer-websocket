...
use crate::handlers::{route_code_to_name, CODE_NO_ROUTE};
...
impl Route {
    pub const fn from(code: u16) -> Self {
        Route {
            code: Some(code),
            name: None,
        }
    ...
    pub fn get_name(&self) -> String {
        if let Some(name) = &self.name {
            ...
            route_code_to_name(self.code.unwrap_or(0)).to_owned()
        }
    }
...
impl Msg {
    ...
    pub fn decode(buffer: &[u8]) -> Result<Msg> {
        ...
        if Self::msg_has_route(msg_type) {
            if compress_route != 0 {
                let route_bytes: [u8; 2] = bytes[offset..offset + 2].try_into()?;
                let route_code = u16::from_be_bytes(route_bytes);
                route = Route::from(route_code);
                offset += 2;
            } else {

...
namespace Service
{
    public class HttpLogin : AbstractDataAccess
    {
        ...
        {
#if SERVICE_LOG
            Log.D("HttpLogin AsyncLogin", account, password);
#endif

            if (m_Request != null && m_Request.IsRunning)
            {
#if SERVICE_LOG
                Log.W("HttpLogin AsyncLogin", "request is in progress, clear");
#endif
                m_Request.Clear();
            ...
            var result = await m_Request.AsyncJsonPost<string, string>(App.Constants.HTTP_LOGIN, requestData);

            if (!string.IsNullOrEmpty(m_Request.Error) && string.IsNullOrEmpty(result))
            {
                Log.E("HttpLogin AsyncLogin", m_Request.Error);
                return null;
            }
            ...
            if (json == null)
            {
                Log.E("HttpLogin AsyncLogin", "result json parse error");
                return null;
            }
            ...
            {
                Log.E("HttpLogin AsyncLogin", "result err", errCode, Configs.Instance.ServerErrorMsg.GetErrMsg(errCode));
                return null;
            }
